...
FinsightRAG 采用模块化的多模态 RAG 流水线：

1. OCR 将金融 PDF 转换为结构化的页级输出。
2. 文本内容被切分为语义上更稳定的文本块。
3. 表格和图片根据布局 bbox 从 PDF 页面中裁剪出来。
4. 视觉语言模型为表格和图片生成标题、描述和结构化内容。
5. 文本、表格和图片证据分别建立索引。
6. 检索阶段优先使用文本证据，当文本证据不足时补充表格和图片证据。
7. 最终答案基于证据包生成，而不是直接依赖原始 PDF 内容。

该设计的目标是让金融文档问答过程更加可追溯、可调试、可复现。
...
本项目受 MultiFinRAG 论文启发，目标是构建一个轻量、可复现、工程友好的金融文档多模态 RAG 基线系统。
